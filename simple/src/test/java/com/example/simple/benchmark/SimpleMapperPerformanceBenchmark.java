...
public class SimpleMapperPerformanceBenchmark {
    ...
    @Benchmark
    public User kyraSelectById(BenchmarkState state) {
        return state.kyraMapper.selectById(state.selectId);
    }
    ...
    @Benchmark
    public int kyraInsertOne(BenchmarkState state) {
        return state.kyraMapper.insert(state.newKyraUser(state.kyraInsertId.incrementAndGet()));
    }
    ...
    @Benchmark
    public List<User> kyraSelectByAgeRange(BenchmarkState state) {
        return state.kyraMapper.selectByAgeRange(state.minAge, state.maxAge);
    }
    ...
    @Benchmark
    public int kyraUpdateById(BenchmarkState state, UpdateTargetState targets) {
        long id = targets.kyraUpdateId;
        User user = state.newKyraUser(id);
        user.setName("updated-" + id);
        return state.kyraMapper.updateById(user);
    }
    ...
    @Benchmark
    public int kyraDeleteById(BenchmarkState state, DeleteTargetState targets) {
        return state.kyraMapper.deleteById(targets.kyraDeleteId);
    }
    ...
    @Benchmark
    public Page<User> kyraPage(BenchmarkState state) {
        return state.kyraMapper.selectPage(Paging.of(1, 20), state.minAge, state.maxAge);
    }
    ...
    @Benchmark
    public int kyraBatchInsert(BenchmarkState state) {
        long base = state.kyraBatchBase.getAndAdd(100);
        return state.kyraMapper.insert(state.newKyraUsers(base, 100));
    }
    ...
    public static class BenchmarkState {
        ...
        public void setup() throws Exception {
            ...
            dataSource.setURL("jdbc:h2:mem:kyra-benchmark;MODE=MYSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1");
            dataSource.setUser("sa");
            dataSource.setPassword("");

            initializeSchema(dataSource);
            this.dataSource = dataSource;
            this.kyraMapper = createKyraMapper(dataSource);
            ...
            this.minAge = 20;
            this.maxAge = 35;
            this.kyraInsertId = new AtomicLong(1_000_000L);
            ...
            this.kyraDeleteId = new AtomicLong(100_001L);
        ...
        private UserMapper createKyraMapper(DataSource dataSource) {
        ...
        private User newKyraUser(long id) {
            return new User(id, "user-" + id, 18 + (int) (id % 40), "login-" + id);
        }

        private List<User> newKyraUsers(long base, int size) {
            return java.util.stream.LongStream.range(base, base + size)
                    .mapToObj(this::newKyraUser)
                    .toList();
        }
    ...
    public static class UpdateTargetState {
        ...
        public void setup(BenchmarkState state) {
            this.kyraUpdateId = state.existingUpdateId(state.kyraUpdateId);
    ...
    public static class DeleteTargetState {
        ...
        public void setup(BenchmarkState state) throws Exception {
            this.kyraDeleteId = state.kyraDeleteId.getAndIncrement();
            ...
            state.ensureUser(kyraDeleteId, "kyra-delete");
