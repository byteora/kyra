...
(cd "${ROOT_DIR}" && bash gradlew :simple:compileJava --info | tee /tmp/kyra-mapper-incremental.log)
grep -q "Incremental compilation" /tmp/kyra-mapper-incremental.log
cp "${mapper_backup}" "${MAPPER_FILE}"

echo "== Entity-only incremental compile =="
append_probe "${ENTITY_FILE}"
(cd "${ROOT_DIR}" && bash gradlew :simple:compileJava --info | tee /tmp/kyra-entity-incremental.log)
grep -q "Incremental compilation" /tmp/kyra-entity-incremental.log
cp "${entity_backup}" "${ENTITY_FILE}"

echo "== Verify generated artifacts =="
test -f "${ROOT_DIR}/simple/build/classes/java/main/gen/kyra-generated.idx"
test -f "${ROOT_DIR}/simple/build/classes/java/main/gen/kyra-scan.idx"
